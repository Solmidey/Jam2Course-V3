...
<input placeholder="paste jam room url" className="input" value={jamUrl} onChange={e=>setJamUrl(e.target.value)} />
<button className="btn" onClick={onGenerate}>Generate</button>
<a className="cta" href="#via-jam">Via Jam</a>
</div>
</header>
...
<CourseGrid courses={courses} onOpen={setSelected} />
...
<div className="small-muted" style={{marginTop:8}}>{rooms.length} rooms</div>
</div>
))}
</div>
</section>
...
<CoursePage course={selected} onBack={()=>setSelected(undefined)} />
)}
</main>
</div>
)
